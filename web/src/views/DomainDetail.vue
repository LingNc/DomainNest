          ...
          >
            ...
            <el-table-column :label="$t('common.actions')" min-width="220" fixed="right">
              <template #default="{ row }">
                ...
                <template v-else>
                  <template v-if="row.own_node_id === domain?.id && row.host === '@' && auth.user?.id === domain?.owner_id && !domain?.parent_id">
                  ...
                  <el-button v-if="!row.own_node_id && row.host !== '@'" link type="primary" size="small" @click="handleMakeIndependent(row)">{{ $t('domainDetail.convertToNode') }}</el-button>
