...
func Setup(cfg *config.Config, db *gorm.DB, authService *service.AuthService,
	...
	friendHandler := handler.NewFriendHandler(friendService, notificationService, messageService, db)
	...
	domains.Use(middleware.JWTAuth(cfg.JWT.Secret), middleware.OnlineTracker(db))
	{
		...
		domains.GET("/:id/archived-children", domainHandler.ListArchivedChildren)
		domains.POST("/:id/archived-children/:childId/restore", domainHandler.RestoreArchivedChild)
